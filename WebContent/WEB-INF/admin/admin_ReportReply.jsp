	...
	<div class="llayout-main">
		...
		<div class="layout-right-block">
			...
			<th>&gt; 관리 &gt; 신고내역확인
						...
						<option value="/beepPro/admin/admin_report_list.do"  >게시글</option>
						<option value="/beepPro/admin/admin_reportReply_list.do" selected="selected">댓글	</option>
						</select></th>
			</thead>
			...
			<tbody>
			<c:forEach items="${replyviewData.admin_reportReply_List}" var="reportReply">
				<tr>
					<th scope="row">${reportReply.report_seq }</th>
					<td>${reportReply.report_name }</td>
					<td>${reportReply.name }</td>					
					<td><a href="/beepPro/admin/admin_reportRelpy_content.do?report_seq=${reportReply.report_seq }">${reportReply.contents }</a></td>
					<td>${reportReply.id }</td>
					<td>${reportReply.dates }</td>
			...
			<div style="text-align: center">
				<c:forEach var="pageNum" begin="1"	end="${ replyviewData.pageTotalCount }">
					<c:if test="${ pageNum eq replyviewData.currentPageNumber }">
						<span style='color: red'>${ pageNum }</span>
					</c:if>
					<c:if test="${ not (pageNum eq replyviewData.currentPageNumber) }">
						<a href="/beepPro/admin/admin_reportReply_list.do?page=${ pageNum }">${ pageNum }</a>
					</c:if>
				</c:forEach>
			</div>
			</div>
