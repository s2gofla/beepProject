...
<!-- 줭신차려미이이이니ㅣㄴ너저엉 -->
	...
	<div class="llayout-main">
		...
		<div class="layout-right-block">
			...
			<div id="write">
			<%-- 
				<a href="/beepPro/community/free_write.do?id=${authUser.id}">
			...
			<table class="board">
				...
				<c:forEach items="${view.messageList }" var="msg">
					<tr>
						<th>${msg.no}</th>
						<th><a href="/beepPro/community/free_content.do?fboard_seq=${msg.fboard_seq }&page=${view.currentPageNumber}&searchCondition=${searchCondition}&searchWord=${searchWord}" class="listtitle">${msg.title }</a>
 ...
 -->
			...
			<div class="pagination" style="text-align: center">
					<c:if test="${view.prev }"><a href="/beepPro/community/free_list.do?page=${ view.pageBlockStart-1 }&searchCondition=${searchCondition}&searchWord=${searchWord}">
				...
				<c:forEach var="pageNum" begin="${ view.pageBlockStart }"	end="${ view.pageBlockEnd }" >
						...
						<c:if test="${ not (pageNum eq view.currentPageNumber) }">
							<c:if test=""></c:if>
							<a href="/beepPro/community/free_list.do?page=${ pageNum }&searchCondition=${searchCondition}&searchWord=${searchWord}">${ pageNum }</a>
						</c:if>
				</c:forEach>
					<c:if test="${view.next }"><a href="/beepPro/community/free_list.do?page=${ view.pageBlockEnd+1 }&searchCondition=${searchCondition}&searchWord=${searchWord}">
...
<!-- 
 ...
 -->
			<div>
				<form action="/beepPro/community/free_list.do" class="box">
