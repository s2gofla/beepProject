...
$(function(){
	...
	function showHtml(data) {
        ...
        $.each(data, function(index, item) {
            ...
            html += "<br />";
            html += "<div id='disease_combox"+item.seq+"'>";
			...
			html += "<form action='/beepPro/community/disease_ccomwrite.ajax' method='post' id='ccomment_form"+item.seq+"' class='disease_ccomment'>";
...
*/
	            ...
	            function commentDelete(deletebtn){
	                if(confirm("정말 삭제하시겠습니까")){
	                   ...
	                   var commentbox = "#disease_combox"+seq;
	             ...
	             function commentDeleteAjax(seq){
	                ...
	                $.ajax({
	                   type : 'POST',
	                   url : "/beepPro/community/disease_comdelete.ajax?" + "seq=" + seq,
	                   data : data,
	                   success : function(data) {
        ...
        // 답글 삭제
		            function ccommentDelete(deletebtn){
		                if(confirm("정말 삭제하시겠습니까")){
		                   var id = deletebtn.id;
		                   var direply_seq = id.substring(id.indexOf("ete")+3,id.length);
		                   var ccommentbox = "#ccomentbox"+direply_seq;
		                   console.log(direply_seq);
		                   console.log(ccommentbox);
		                   ccommentDeleteAjax(direply_seq);
		                   $(ccommentbox).remove();
		                }else{
		                   return false;
		                }
		             }
		             function ccommentDeleteAjax(direply_seq){
		                var data = null;
		                $.ajax({
		                   type : 'POST',
		                   url : "/beepPro/community/disease_ccomdelete.ajax?" + "direply_seq=" + direply_seq,
		                   data : data,
		                   success : function(data) {
		                      console.log("답변삭제 보내기 성공");
		                      console.log(data);
